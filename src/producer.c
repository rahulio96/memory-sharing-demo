...
void InitShm(int bufSize, int itemCnt) {
    int in = 0;
    int out = 0;
    const char* name = "OS_HW1_rahulGupta";  // Name of shared memory object to
...
void Producer(int bufSize, int itemCnt, int randSeed) {
    ...
    for (int i = 0; i < itemCnt; i++) {
        ...
        // Use the provided function GetRand() to generate a random number in
        // the specified range
        int val = GetRand(0, 3000);

        // Use the following print statement to report the production of an item
        // where i is the item number, val is the item value, in is its index in
        // the bounded buffer:
