...
int main() {
    const char* name = "OS_HW1_rahulGupta";  // Name of shared memory block to
                                             // be passed to shm_open
    ...
    // Write code here to read the four integers from the header of the shared
    // memory block These are: bufSize, itemCnt, in, out Just call the functions
    // provided below like this:
    bufSize = GetBufSize();
    itemCnt = GetItemCnt();
    in = GetIn();
    out = GetOut();
    ...
    for (int i = 0; i < itemCnt; i++) {
        // Use the functions provided below to get/set the values of shared
        // variables in, out, bufSize
        // **Extremely Important: Remember to set the value of any shared
        // variable you change locally
        in = GetIn();
        out = GetOut();
        ...
        // Use the provided function ReadAtBufIndex() to read from the bounded
        // buffer
        int val = ReadAtBufIndex(out);

        // Use the following print statement to report the consumption of an
        // item where i is the item number, val is the item value, out is its
        // index in the bounded buffer
